...
let expName = 'DNN_noise_naming';  // from the Builder filename that created this script
...
function updateInfo() {
  ...
  psychoJS.setRedirectUrls('https://app.prolific.co/submissions/complete?cc=8370189F', ''); //*****
...
function downloadFiles() {
   ...
   var master_cond_file = [
       { name: 'cond_file_list_2B_' + pid + '.csv', path: 'resources/cond_file_list_2B_' + pid + '.csv' }
     ];
   var cond_files = new Array(nBlocks);
   for (var i = 0; i < nBlocks; i++) {
       cond_files[i] = { name: 'im_list_2B_' + pid + '_' + i.toString() + '.csv', path: 'resources/im_list_2B_' + pid + '_' + i.toString() + '.csv' };
   }
   var im_files = new Array(n_files);
   for (var i = 0; i < n_files; i++) {
       im_files[i] = { name: 'Images/2B_' + pid + '/stim_2B_' + pid + '_' + i.toString() + '.png', path: 'resources/Images/2B_' + pid + '/stim_2B_' + pid + '_' + i.toString() + '.png' };
   }
   
   // download resources
   var rsrc = master_cond_file.concat(cond_files);
   psychoJS.downloadResources(rsrc);
   psychoJS.downloadResources(im_files.slice(0,33)); // seems we need to do this because of weird bug...
   psychoJS.downloadResources(im_files.slice(30,66));
   psychoJS.downloadResources(im_files.slice(63,100));
   psychoJS.downloadResources(im_files.slice(99));
...
function blocksLoopBegin(thisScheduler) {
  ...
  blocks = new TrialHandler({
    ...
    nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
    extraInfo: expInfo, originPath: undefined,
    trialList: 'cond_file_list_2B_' + expInfo["participant"] + '.csv',
...
function trials_2LoopBegin(thisScheduler) {
  ...
  trials_2 = new TrialHandler({
    ...
    nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
    extraInfo: expInfo, originPath: undefined,
    trialList: 'im_list_2B_' + expInfo["participant"] + '_4.csv', ////*****************
