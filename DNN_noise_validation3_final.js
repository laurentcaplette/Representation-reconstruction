/**************************
 * Dnn_noise_validation3 Experiment *
 **************************/
...
let expName = 'DNN_noise_validation3';  // from the Builder filename that created this script
...
for (var i = 0; i < n_im_files; i++) {
  im_files[i] = { name: 'Images/valid3_' + i.toString() + '.png', path: 'resources/Images/valid3_' + i.toString() + '.png' };
}
...
function updateInfo() {
  ...
  psychoJS.setRedirectUrls('******** TO CHANGE *********', ''); //     ********
...
function downloadFiles() {
   ...
   var master_cond_file = [
       { name: 'cond_file_list_valid3_' + pid + '.csv', path: 'resources/cond_file_list_valid3_' + pid + '.csv' }
     ];
   var cond_files = new Array(nBlocks);
   for (var i = 0; i < nBlocks; i++) {
       cond_files[i] = { name: 'label_list_valid3_' + pid + '_' + i.toString() + '.csv', path: 'resources/label_list_valid3_' + pid + '_' + i.toString() + '.csv' };
   }
...
function experimentInit() {
  ...
  instructions_text_2 = new visual.TextStim({
    ...
    text: "We will show you pictures that might depict an object or other visual concept (more or less clearly). A concept may occupy the whole image or only a part of the image, and may be depicted many times across the image. We will ask you to write a one-word label describing what you see (or think you see) in the image. \n\nWe will show each picture for 2 seconds. Then, it will disappear and you will be able to type your label. Press ENTER to confirm and go to the next image. Please restrict each response to one word. Please answer quickly. \n\nPress SPACE to continue.",
    font: 'Arial',
    units: 'height',
    pos: [0, 0], height: 0.03,  wrapWidth: undefined, ori: 0,
    color: new util.Color('black'),  opacity: 1,
...
function blocksLoopBegin(thisScheduler) {
  ...
  blocks = new TrialHandler({
    ...
    trialList: 'cond_file_list_valid3_' + expInfo["participant"] + '.csv',
...
function trials_2LoopBegin(thisScheduler) {
  ...
  trials_2 = new TrialHandler({
    ...
    nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
    extraInfo: expInfo, originPath: undefined,
    trialList: 'label_list_valid3_' + expInfo["participant"] + '_4.csv', ////*****************
...
function trialRoutineBegin(trials) {
  return function () {
    ...
    stimulus.setImage('Images/valid3_'+im_nb.toString()+'.png');
    // keep track of which components have finished
    trialComponents = [];
...
function typingRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'trial'-------
    t = 0;
    currLabel = 2; // just start as if was 3rd label (no need to edit code)
    trialClock.reset(); // clock
    frameN = -1;
